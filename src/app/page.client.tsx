...
export default function Home() {
  ...
  const { data: indexerResponse, error: indexerError, isLoading: indexerLoading } = useSWR<ApiResponse>(
    INDEXER_API_ENDPOINT,
    fetcher,
    { refreshInterval: 5000 }
  );

  const { data: tokenResponse, error: tokenError, isLoading: tokenLoading } = useSWR<PaginatedTokenListResponse>(
    `${TOKEN_API_ENDPOINT}&limit=${ITEMS_PER_PAGE}&offset=${(currentPage - 1) * ITEMS_PER_PAGE}&v=1`,
    fetcher
  );
  ...
  const percentIndexed = indexerResponse && indexerResponse.data
    ? ((indexerResponse.data.trackerBlockHeight / indexerResponse.data.latestBlockHeight) * 100).toFixed(2)
    : null;
