...
export function getGuardsP2TR(
	guardType: GuardType = GuardType.Transfer
): {
	p2tr: string
	tapScript: string
	cblock: string
	...
	const tapleafKeyBurnGuard = Tap.encodeScript(burnGuard.lockingScript.toBuffer())
	const tapleafKeyTransferGuard = Tap.encodeScript(transferGuard.lockingScript.toBuffer())
...
export function getClosedMinterContract(issuerAddress: string, genesisId: ByteString) {
...
export function getDummySigner(privateKey?: bsv.PrivateKey | bsv.PrivateKey[]): TestWallet {
	if (global.dummySigner === undefined) {
		...
		// let pk = bsv.PrivateKey.fromRandom(bsv.Networks.testnet)
		// console.log('pk', pk)
		// global.dummySigner = new TestWallet(pk, new DummyProvider())
	}
...
export function p2tr2Address(p2tr: string | btc.Script, network: SupportedNetwork) {
