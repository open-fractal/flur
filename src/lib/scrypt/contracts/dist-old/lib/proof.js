...
const txToTxHeader = function(tx) {
	...
	for (let index = 0; index < outputNumber; index++) {
		...
		// outputScriptLenList[index] = (0, scrypt_ts_1.toHex)(varuint.encode(scriptLen).buffer)
	...
	return {
		...
		//inputCount: (0, scrypt_ts_1.toHex)(varuint.encode(inputNumber).buffer),
		...
		// outputCount: (0, scrypt_ts_1.toHex)(varuint.encode(outputNumber).buffer),
