...
export class WalletService {
	...
	async signFeeInputs(txs: btc.Transaction[]) {
		const psbts = txs.map(tx => tx2PSBT(tx))
		const toSignInputsArray: Array<{
		...
		// Process each PSBT
		for (let txIndex = 0; txIndex < psbts.length; txIndex++) {
			const psbt = psbts[txIndex]
			...
			toSignInputsArray.push({
				autoFinalized: false,
				toSignInputs
			})
		}

		// Sign all PSBTs at once
		const signedPsbtHexs = await window.unisat.signPsbts(
			psbts.map(psbt => psbt.toHex()),
			toSignInputsArray
		)

		// Process the signed PSBTs
		for (let txIndex = 0; txIndex < txs.length; txIndex++) {
			const tx = txs[txIndex]
			const signedPsbt = bitcoinjs.Psbt.fromHex(signedPsbtHexs[txIndex])
	...
	async signContract(tx: btc.Transaction, tapContract: TaprootSmartContract): Promise<string[]> {
		const psbt = tx2PSBT(tx)

		const xpubkeyContract = tapContract.lockingScriptHex.slice(4)
		const xpubkeyFee = await this.getXOnlyPublicKey()
				...
				if (pkh === xpubkeyContract) {
					psbt.updateInput(i, {
						...
						tapLeafScript: [
							{
								leafVersion: 192,
								script: tapContract.contractScriptBuffer,
								controlBlock: tapContract.cblockBuffer
							}
						],
						sighashType: 1
					})
